...
export default function useAccountantWorker() {
  ...
  const connect = useCallback(() => {
    ...
    ws.onmessage = (e) => {
      try {
        ...
        if (msg.type === "qr") { setQrImage(msg.image); setZaloConnected(false); setZaloError(null); }
        if (msg.type === "zalo_ready") { setQrImage(null); setZaloConnected(true); setZaloError(null); }
        ...
        if (msg.type === "zalo_logout" || msg.type === "zalo_expired") {
          setZaloConnected(false); setZaloGroups([]); setSelectedGroups([]); setZaloError(null);
        }
  ...
  return {
    wsConnected, zaloConnected, qrImage, zaloError,
