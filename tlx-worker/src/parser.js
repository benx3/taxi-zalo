...
// Các bến xe lớn được nhận diện rõ — "bx/bxe <tên>" này là bến xe, không phải bao xe
// Chỉ whitelist các bến xe thực sự; mọi "bx/bxe <địa danh khác>" → bao xe
const BEN_XE_RE = /\bbxe?\s*(?:mỹ\s*đình|my\s*dinh|giáp\s*bát|giap\s*bat|nước\s*ngầm|nuoc\s*ngam|gia\s*lâm|gia\s*lam|yên\s*nghĩa|yen\s*nghia)\b/i;
...
function cleanPlace(s) {
  ...
  do {
    ...
    s = s
      ...
      .replace(/^\s*(csct|cs ct|cnct|cn ct|snct|sn ct|free+|fr+ee|fer+|frr+|fii+|fee+|fri+|vtri|vt|vị trí|yc|ycvt|dự|sm|sd|sáng mai|ngày mai|mai|gấp|gap)(?=[\s.:,_-]|$)[\s.:,_-]*/i, "")
  ...
  return s
    ...
    .replace(/\b(free+|fr+ee|fer+|frr+|fii+|fee+|fri+|tg|tgct|tgian|ki\d+|sd|cl|0[.,]5)\b.*$/i, "")
...
export function parseTrip(raw) {
  ...
  return {
    ...
    free: /\b(?:fre+|frr+|fii+|fer+|fee+|fri+)\b/i.test(text),
    explicitPoints,
