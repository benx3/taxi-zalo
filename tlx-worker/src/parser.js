...
export function parseType(t) {
  ...
  const AIRPORT_RE = /sân\s*bay|sân\s*qu[oô]c\s*t[eế]|sân\s*qu[oô]c\s*n[oô]i|bay\s*qu[oô]c\s*t[eế]|bay\s*qu[oô]c\s*n[oô]i|n[oô]i\s*b[àa]i|noi\s*bai|\bnb\b|\bsb\b|\bt1\b|\bt2\b|s[aả]nh|h[aạ]\s*s[aâ]n|h[aạ]\s*c[aá]nh|c[aấ]t\s*c[aá]nh|ch[uư][aâ]n\s*b[iị]\s*h[aạ]|đ[aá]p\s*xu[oô]ng/;
